...
public class PersistenceAdaptersConfiguration {
    ...
    private JpaModels.EligibilityAssessmentEntity toAssessmentEntity(EligibilityAssessment value) { var e=new JpaModels.EligibilityAssessmentEntity(); e.id=value.id(); e.candidateProfileId=value.candidateProfileId(); e.jobPostingId=value.jobPostingId(); e.status=value.status(); value.ruleResults().forEach((key,result)->e.ruleResults.put(key.name(),Map.of("status",result.status().name(),"explanation",result.explanation()))); e.evidenceIds=new ArrayList<>(value.evidenceIds()); e.evaluatorVersion=value.evaluatorVersion(); e.assessedAt=value.assessedAt(); e.profileVersion=value.profileVersion(); e.jobContentFingerprint=value.jobContentFingerprint(); return e; }
    private EligibilityAssessment toAssessment(JpaModels.EligibilityAssessmentEntity e) { var results=new EnumMap<RuleType,RuleResult>(RuleType.class); e.ruleResults.forEach((key,value)->results.put(RuleType.valueOf(key),new RuleResult(EligibilityStatus.valueOf(value.get("status")),value.get("explanation")))); return new EligibilityAssessment(e.id,e.candidateProfileId,e.jobPostingId,e.status,results,e.evidenceIds,e.evaluatorVersion,e.assessedAt,e.profileVersion,e.jobContentFingerprint); }
