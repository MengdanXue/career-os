...
public class OfficialExcelImportService {
    ...
    private static void copyAnnouncementFacts(JpaModels.RecruitmentEventEntity target,JpaModels.RecruitmentEventEntity source){if(source==null)return;target.applicationStartsAt=source.applicationStartsAt;target.applicationEndsAt=source.applicationEndsAt;target.ageReferenceDate=source.ageReferenceDate;target.registrationUrl=source.registrationUrl;target.qualificationReviewEndsOn=source.qualificationReviewEndsOn;target.paymentEndsOn=source.paymentEndsOn;target.admissionTicketStartsOn=source.admissionTicketStartsOn;target.admissionTicketEndsOn=source.admissionTicketEndsOn;target.writtenExamOn=source.writtenExamOn;target.writtenExamSubjects=source.writtenExamSubjects==null?new ArrayList<>():new ArrayList<>(source.writtenExamSubjects);target.graduateRule=source.graduateRule;target.overseasDegreeRule=source.overseasDegreeRule;target.experienceEvidenceRule=source.experienceEvidenceRule;target.employmentStatement=source.employmentStatement;target.interviewRule=source.interviewRule;target.graduateRuleJson=source.graduateRuleJson;target.writtenExamState=source.writtenExamState;target.professionalTestState=source.professionalTestState;target.interviewState=source.interviewState;target.interviewOn=source.interviewOn;target.interviewMethod=source.interviewMethod;target.scoreFormula=source.scoreFormula;target.noticeState=source.noticeState;target.applicationState=source.applicationState;target.qualificationReviewState=source.qualificationReviewState;target.paymentState=source.paymentState;target.admissionTicketState=source.admissionTicketState;target.physicalExamState=source.physicalExamState;target.investigationState=source.investigationState;target.publicationState=source.publicationState;target.appointmentState=source.appointmentState;target.physicalExamRule=source.physicalExamRule;target.investigationRule=source.investigationRule;target.publicationRule=source.publicationRule;target.appointmentRule=source.appointmentRule;}
