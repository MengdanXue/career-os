...
public class OfficialExcelImportService {
    ...
    public ImportResult importWorkbook(InputStream input,ImportCommand command) throws Exception {
        ...
        int recognizedSheets=0; var errors=new ArrayList<RowError>(); var seen=new HashSet<String>(); var normalizedJobs=new ArrayList<NormalizedJob>();var rowEvidence=new ArrayList<RowEvidence>();
            ...
            for(int sheetIndex=0;sheetIndex<workbook.getNumberOfSheets();sheetIndex++){
                ...
                for(int rowIndex=header.rowIndex()+1;rowIndex<=sheet.getLastRowNum();rowIndex++){
                    ...
                    try{
                        ...
                        String degreeRequirement=value(row,header.columns(),formatter,"学位","学位要求");
        ...
        if(workbookEvidenceId!=null){for(int index=0;index<Math.min(result.jobIds().size(),rowEvidence.size());index++){var located=rowEvidence.get(index);workbookEvidence.replaceJobFacts(workbookEvidenceId,result.jobIds().get(index),located.sheet(),located.row(),located.fields(),located.sourceColumns());}}
    ...
    private static Map<String,String> sourceColumns(Header header){var result=new LinkedHashMap<String,String>();column(result,header,"externalJobCode","岗位代码","岗位编号","职位代码","岗位序号","序号");column(result,header,"title","岗位名称","招聘岗位","招聘岗位名称","岗位");column(result,header,"organizationName","招聘单位","单位名称","用人单位","招聘主体","用人学院（部门）","用人学院部门");column(result,header,"headcount","招聘人数","人数","计划人数","计划数");column(result,header,"employmentType","用工性质","编制性质","岗位性质","聘用形式");column(result,header,"supervisingDepartment","主管单位（部门）","主管单位","主管部门");column(result,header,"jobCategory","岗位类别","岗位类型");column(result,header,"jobGrade","岗位等级","岗位级别");column(result,header,"educationRequirementText","学历","学历要求","最低学历");column(result,header,"degreeRequirement","学位","学位要求");column(result,header,"majorRequirementText","专业","专业要求","所学专业","学科/专业要求","专业/学科方向","学科专业要求");column(result,header,"ageRequirementText","年龄","年龄要求");column(result,header,"genderRequirement","性别要求","性别");column(result,header,"candidateScope","招聘对象","人员范围","对象范围");column(result,header,"otherRequirements","其他条件","其他资格条件或要求","资格条件","其他要求","备注");column(result,header,"interviewRatio","经笔试入围比例","入围面试比例","面试比例");column(result,header,"professionalTestRequired","是否设置专业（业务、技能、心理素质）测试","是否设置专业测试","专业测试");column(result,header,"contactPhone","招聘单位咨询电话","咨询电话","联系电话");return Collections.unmodifiableMap(result);}
