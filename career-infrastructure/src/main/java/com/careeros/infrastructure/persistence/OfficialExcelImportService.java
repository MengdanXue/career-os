...
public class OfficialExcelImportService {
    ...
    private static EmploymentType employmentType(String text){if(blank(text))return EmploymentType.UNKNOWN;if(text.contains("劳务派遣"))return EmploymentType.LABOR_DISPATCH;if(text.contains("人事代理"))return EmploymentType.PERSONNEL_AGENCY;if(text.contains("项目"))return EmploymentType.PROJECT_BASED;if(text.contains("合同"))return EmploymentType.CONTRACT;if(text.contains("事业编")||text.contains("编制内"))return EmploymentType.ESTABLISHMENT;if(text.contains("员额")||text.contains("备案制"))return EmploymentType.AUTHORIZED_HEADCOUNT;if(text.contains("校聘")||text.contains("院聘"))return EmploymentType.SCHOOL_HIRED;if(text.contains("国企正式")||text.contains("集团正式"))return EmploymentType.STATE_OWNED_REGULAR;return EmploymentType.UNKNOWN;}
