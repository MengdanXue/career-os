...
| `FRESH_GRADUATE_STATUS` | 公告的应届条款（未落实工作单位 / 社保限制）vs 候选人的报名时声明 | 待确认 |
...
公告限定“未落实工作单位”或“无社保缴纳记录”时，要求的是**报名当天**的状态，而报名还没发生。
候选人今天无法确认一个未来时点的事实，只能声明一个打算，因此 `CandidateProfile` 上这两项
存的是声明：

| 值 | 含义 | 判定 |
| --- | --- | --- |
| `DECLARED_MET` | 声明报名时将满足 | `CONDITIONAL`——把还没兑现的未来当成既成事实是基线禁止的 |
| `DECLARED_NOT_MET` | 声明报名时不会满足 | `INELIGIBLE`——这是一个已经确定、不会变回去的事实 |
| `UNDECLARED` | 尚未声明（默认值） | `NEEDS_CONFIRMATION` |

对应两个确认键 `EMPLOYER_SETTLEMENT_AT_APPLICATION`、`SOCIAL_INSURANCE_AT_APPLICATION`，
资料页各有一个下拉。默认 `UNDECLARED`：基线 §2.2 把这两项列为“不得由模型猜测”的字段，
缺省必须是“没说”而不是“满足”，因此即使被批量确认，`UNDECLARED` 仍然只得出待确认。

岗位侧读的是招聘事件上已解析的 `GraduateEligibilityRule`（`requiresNoEmployer`、
`restrictsSocialInsurance`）。该对象为 null 表示公告里没有应届条款，不构成限制——解析失败
或未采集同样是 null，所以说明里写的是“公告未解析出应届身份限制”，而不是断言“没有限制”。
条款本身 `evidenceState` 不是 `CONFIRMED` 时不拿它判定，先让人核对原文。
两项限定各自独立判定后取最严重的一条：工作单位那条是条件可报、社保那条是不可报时，
结论必须是不可报。

**尚未实现的规则**：户籍。岗位侧完全没有模型，要从公告自由文本里识别“限本市户籍”，
