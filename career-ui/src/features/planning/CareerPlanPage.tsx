...
function RouteCard({ route, rank }: { route: Route; rank: number }) {
  return <article className="route-card">
    <header><span className="route-rank">路线 {rank}</span><span className={`evidence-chip evidence-${route.evidenceStrength.toLowerCase()}`}>{evidenceLabels[route.evidenceStrength]}</span></header>
    <h2>{route.label}</h2>
    <div className="decision-index"><strong>{route.priorityScore}</strong><span>{route.priorityLabel}</span></div>
    <dl className="route-counts"><div><dt>历史岗位</dt><dd>{route.historicalJobCount}</dd></div><div><dt>独立招聘</dt><dd>{route.eventCount}</dd></div><div><dt>正式用工</dt><dd>{route.formalJobCount}</dd></div></dl>
...
export function CareerPlanPage() {
  ...
  const keyAgeWindow = data?.ageWindows.find(window => window.year === 2028 && window.maximumAge === 35) ?? data?.ageWindows.find(window => window.eligible)
  const primaryRoute = data?.recommendedRoutes[0]

  return <main className="planning-page page-frame wide-frame">
    <AsyncState loading={plan.isLoading} error={plan.error}>
      {data && <>
        <header className="planning-hero">
          <div><p className="eyebrow">CAREER ROUTE · 2027 规划</p><h1>我的半体制规划</h1><p className="planning-current">当前：{data.candidateSnapshot.educationSummary}</p></div>
        ...
        <section className="plan-conclusion" aria-labelledby="plan-conclusion-title">
          <p className="section-number">先看结论</p><h2 id="plan-conclusion-title">主攻 {primaryRoute?.label ?? '事业单位技术岗'}</h2>
          <p>{keyAgeWindow?.label ?? '年龄窗口将在历史岗位规则补齐后显示'}；{data.recruitmentWindows[0]?.label ?? '持续关注官方公告'}。先把工作经历证据和留服认证路径准备好。</p>
        ...
        <section className="plan-section history-section"><header><p className="section-number">2024—2026 官方历史切片</p><h2>历史供给是参考规模，不是可报数量</h2></header>
          <div className="history-ledger" role="table" aria-label="年度历史岗位">
            ...
            {data.historicalSummary.map(summary => <div className="history-row" role="row" key={summary.year}><strong>{summary.year}</strong><span>{summary.jobCount}</span><span>{summary.eventCount}</span><span>{summary.formalJobCount}</span><span>{summary.coverageComplete ? '已完成' : '待补齐'}</span></div>)}
          </div>
        </section>

        <section className="plan-section" id="routes"><header><p className="section-number">路线比较</p><h2>按资格准备度、经历复用和稳定性排序</h2></header><div className="route-grid">{data.recommendedRoutes.map((route, index) => <RouteCard route={route} rank={index + 1} key={route.code} />)}</div></section>
        ...
        <section className="plan-section evidence-section"><header><p className="section-number">官方依据</p><h2>代表岗位可下钻查看完整详情</h2></header><div className="evidence-list">{data.recommendedRoutes.flatMap(route => route.representativeJobs.map(job => ({ ...job, route: route.label }))).map(job => <article key={job.jobId}><span>{job.year} · {job.route}</span><h3><Link to={`/opportunities/${job.jobId}`}>{job.title} · {job.organizationName}</Link></h3><a href={job.sourceUrl} target="_blank" rel="noreferrer">打开官方来源 ↗</a></article>)}</div></section>
      </>}
    </AsyncState>
  </main>
}
