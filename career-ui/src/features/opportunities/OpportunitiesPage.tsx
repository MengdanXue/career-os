...
export function OpportunitiesPage({ initialTier }: { initialTier?: OpportunityTier }) {
  ...
  return <main className="opportunities-page page-frame wide-frame">
    ...
    <AsyncState loading={decisions.isLoading} error={decisions.error} empty={false}>
      <div className="opportunity-layout"><section><div className="queue-summary"><span>{decisions.data?.total ?? 0} 个岗位</span><span>第 {(decisions.data?.page ?? 0) + 1} 页</span></div><OpportunityQueue decisions={decisions.data?.items ?? []} selectedId={selected?.decisionId} onOpen={openDossier} /></section>{selected ? <JobDossier decision={selected} onClose={closeDossier} /> : <aside className="dossier-placeholder"><p className="section-number">岗位档案</p><strong>选择一个岗位查看证据链</strong><p>硬性阻断、分项理由、证据覆盖与计算版本会集中显示在这里。</p></aside>}</div>
    </AsyncState>
  </main>
}
