...
it('blocks decisions when the server says facts are not ready even if old local state says confirmed', async () => {
  const candidate = {
  ...
  expect(screen.queryByRole('heading', { name: '决策首页' })).not.toBeInTheDocument()
})
