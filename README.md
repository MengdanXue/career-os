...
2026-08-24 的验收基线为：28 个数据库迁移、363 个 Java 测试、44 个前端测试全部通过；真实数据库中目标来源 20 个（已接通 2、部分接通 1、未接通 17），已分析 2 个来源、26 个招聘事件、188 个岗位，其中 129 个岗位达到字段证据完整标准。T1/T2/T3 当前均为 0 是准入门与证据条件的真实结果，不代表没有采集到岗位，也不会阻止系统给出毕业月份、留服认证和笔试基线准备行动。规划接口会为纳入分析的每个岗位返回本人历史实际与 2027 类比结论，数据读取失败时暂停排名而不是把失败结果排入路线。
