...
当前阶段没有引入全网爬虫、登录/CAPTCHA 或自动投递。目标市场目录已登记 20 个浙江/杭州官方来源：浙江省人社、杭州市人社已有 2024—2026 年历史覆盖证据，但在完成新的契约与独立增量验证前不会标记为已接入；杭州电子科技大学、浙江工商大学、杭州市第一人民医院 3 个来源已进入真实采集但仍为部分接入；其余 15 个来源明确标记为未接入。页面不会把登记来源误报为已完成采集。采集结果继续进入确定性 Excel 导入或 HTML/PDF 证据审核管道。
...
2026-08-24 的当前验收数据库已迁移到 V35。真实运行态的 5 个已配置来源为部分接入 4、采集失败 1；其余 15 个目录来源未接入，当前没有任何来源在缺少完整生命周期证据时显示为已接入。浙江省与杭州市人社已有三年覆盖记录，杭电已采集 123 篇历史公告并从 2025 年公告中结构化 60 个目标岗位，浙江工商大学已采集 52 篇公告并识别 2 个目标岗位。杭州市第一人民医院的 2024 年旧招聘子域当前无法建立可信 TLS 连接，因此系统保留官方证据和逐条失败原因，不绕过证书校验、不伪造完整数据。当前岗位库共有 17,459 条记录；这仍不代表浙江/杭州全市场总量。最终回归通过 Java 394 项和前端 45 项测试，共 439 项、0 失败。T1/T2/T3 当前结果受准入门与证据条件约束，不会把原始记录直接包装成可报机会。规划接口会为纳入分析的每个岗位返回本人历史实际与 2027 类比结论，数据读取失败时暂停排名而不是把失败结果排入路线。
