...
class CareerPlanEndToEndTest {
    ...
    void buildsThePlannerFromSeededCandidateAndPersistedOfficialEvidence(
        ...
        mvc.perform(get("/api/v1/candidates/{candidateId}/career-plan", CANDIDATE_ID)
            ...
            .andExpect(jsonPath("$.targetMarketCoverage.targetCount").value(30))
            .andExpect(jsonPath("$.targetMarketCoverage.connected").value(0))
            .andExpect(jsonPath("$.targetMarketCoverage.partial").value(30))
            .andExpect(jsonPath("$.targetMarketCoverage.notConnected").value(0))
